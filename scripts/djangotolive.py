...
def AW():
    ...
    cursor_old.execute("select * from  slum_data_administrativeward;")
    ...
    for f in fetch_data:
        ...
        data = (id,city_id,name,shape,ward_number,description,office_address)
...
def EW():
    old = psycopg2.connect(database='old1',user='postgres',password='softcorner',host='localhost',port='5432')
    cursor_old = old.cursor()
    cursor_old.execute("select * from slum_data_electoralward;")
    fetch_data1 = cursor_old.fetchall()
    for f in fetch_data1:
        id = f[0]
...
def Slum():
    ...
    old_insert.execute("select * from slum_data_slum;")
