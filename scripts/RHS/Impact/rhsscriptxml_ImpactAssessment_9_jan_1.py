...
def printdata():
    ...
    demo=0
    try:
        dataval=mongodb.instances.find({"_xform_id_string" :"aSJM5YWy8LXcmHYL3ZMGgY","group_fz8ww97/Phone_Number": "9604141607"})
        for data in dataval:
            count = count + 1
            ...
            xmldata=""
            xmldata = dict2xml({xform_id_string : jsonformat4(jsonvar)})
            xmlfinal=""
    ...
    for key,value in jsonvar.items():
        ...
        if type(value) == list:
            ...
            if len(str_split)==1:
                ...
                if str(str_split[0]) != "_xform_id_string":
                    list1[str_split[0]]=value
    return {'group_eh0at37': [{'group_oq8lx49': [{'If_children_elderly_defecating': 'Z', 'where_is_the_place_of_urinatio': 'B', 'What_are_the_problems_associat': 'H J K', 'Name_of_the_family_member_001': 'Laxmi', 'What_is_the_reason_of_choosing': 'K', 'What_are_the_problems_associat_001': 'E', 'where_is_the_place_of_urinatio_001': 'B'}
