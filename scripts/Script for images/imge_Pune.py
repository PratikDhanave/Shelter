...
def imagemigration2():
	...
	for i in fetch_slum_ids:
		...
		cursor_FCWCQ.execute("select * from ray_survey_reporttemplatedesiredfact where template_desiredfact_id=27 and report_id=2 ;")
		...
		cursor_TcostQ.execute("select * from ray_survey_reporttemplatedesiredfact where template_desiredfact_id=63 and report_id=2 ;")
...
def imagemigration_Tcost():
	...
	for i in fetch_slum_ids:
		...
		if not Tcost:
			...
			cursor_shelter_insert = shelter.cursor()
			string = 'UPDATE master_rapid_slum_appraisalmigrate4 SET toilet_cost='
			string_query = string + "'"+ str(toilet_cost) +"'"+ ' WHERE id=' + str_id +';'
			print string_query
...
def imagemigration_freq():
	...
	for i in fetch_slum_ids:
		...
		TcostQ = psycopg2.connect(database='old1',user='postgres',password='softcorner',host='localhost',port='5432')
		cursor_TcostQ = TcostQ.cursor()
		cursor_TcostQ.execute("select * from ray_survey_reporttemplatedesiredfact where template_desiredfact_id=27 and report_id=2 ;")
		fetch_data12 = cursor_TcostQ.fetchone()
		TcostQ_id = fetch_data12[0]
		TcostA = psycopg2.connect(database='old1',user='postgres',password='softcorner',host='localhost',port='5432')
		cursor_TcostA = TcostA.cursor()
		...
		if not freq:
			...
			shelter = psycopg2.connect(database='shelter_migrate',user='shelter',password='Sh3lt3rAss0ciat3s',host='45.56.104.240',port='5432')
			cursor_shelter_insert = shelter.cursor()
			string = 'UPDATE master_rapid_slum_appraisalmigrate SET frequency_of_clearance_of_waste_containers='
			string_query = string + "'"+ str(freq1) +"'"+ ' WHERE id=' + str_id +';'
			print string_query
			cursor_shelter_insert.execute(string_query)
			shelter.commit()
